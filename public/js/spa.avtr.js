...
spaAvtr = (function(){
	...
	var
		...
		onTapNav = function(event){
			...
			$target.css({'background':spaUtilb.getRandRgb()});
			updateAvatar($target);
		};
		...
		onSetchatee = function(event,arg_map){
			var
				...
				if(old_chatee){
					$nav.find('.spa-avtr-box[data-id='+old_chatee.cid+']')
		...
		onListchange = function(event){
			...
			people_db().each(function(person,idx){
				...
				$box = $('<div/>').addClass(class_list.join(' '))
									.css(person.css_map)
									.attr('data-id',String(person.id))
									.prop('title',spaUtilb.encodeHtml(person.name))
									.text(person.name)
									.appendTo($nav);
			});
		};
