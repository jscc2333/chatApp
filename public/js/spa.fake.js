...
spaFake = (function(){
	...
	peopleList  = [
		...
		{
			name : 'jack',_id : 'id_03', css_map : {top:100,left:20,'background-color':'rgb(128,192,192)'}
		},{
			name : 'wils',_id : 'id_04', css_map : {top:140,left:20,'background-color':'rgb(192,128,128)'}			
		}
	];
	...
	mockSio = (function(){
		...
		emit_sio = function(msg_type,data){
			...
			if(msg_type === 'updatechat' && callback_map.updatechat){
				setTimeout(function(){
					var user = spaModel.people.get_user();
			...
			setTimeout(function(){
				...
				if(callback_map.updatechat){
					callback_map.updatechat([{
						...
						dest_name : user.name,
						sender_id : 'id_04',
						msg_text : 'hi there'+user.name+'!here'
					}]);
				}
				else{
					emit_mock_msg();
