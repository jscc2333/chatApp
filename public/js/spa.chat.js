...
spaChat =(function(){
	...
	var
		...
		setPxsizes = function(){
			...
			px_per_em = spaUtilb.getEmsize(jqueryMap.$slider.get(0));
			window_height_em = Math.floor(
				($(window).height()/px_per_em) + 0.5
			);
			if(window_height_em<configMap.window_height_min_em){
				opened_height_em = configMap.slider_opened_min_em;
			}else if(window_height_em > configMap.window_height_min_em*2.5){
				opened_height_em = configMap.slider_opened_em;
			}else{
				opened_height_em = configMap.slider_opened_em*0.75;
			}
		...
		writeChat = function(person_name,text,is_user){
			...
			jqueryMap.$msg_log.append(
				'<div class="'+msg_class+'">'+
				spaUtilb.encodeHtml(person_name)+':'+
				spaUtilb.encodeHtml(text)+'</div>');
			scrollChat();
		};
		...
		writeAlert = function(alert_text){
			jqueryMap.$msg_log.append(
				...
				spaUtilb.encodeHtml(alert_text)+'</div>'
			);
			scrollChat();
		};
		...
		onListchange = function(event){
			...
			people_db().each(function(person,idx){
				...
				list_html += '<div class = "spa-chat-list-name'+
								...
								spaUtilb.encodeHtml(person.name)+'</div>';
			});

			if(!list_html){
				list_html = String()+'<div class = "spa-chat-list-note">'+
		...
		onUpdatechat = function(event,msg_map){
			...
			if(!(is_user || sender_id === chatee._id)){
		...
		onLogin = function(event,login_user){
			jqueryMap.$head.css({"pointer-events":"auto"});
			configMap.set_chat_anchor('opened');
		};

		onLogout = function(event,logout_user){
			jqueryMap.$head.css({"pointer-events":"none"});
		...
		removeSlider = function(){
			...
			confimap.set_chat_anchor = null;
			return true;
		};

		// 配置模块参数
		configModule = function(input_map){
			spaUtil.setConfigMap({
				input_map : input_map,
